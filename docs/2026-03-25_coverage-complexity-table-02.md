...
- Estimated overall production line coverage (lower-bound): **2.16%**
  - Calculation: executed lines 60 / total production LOC 2783
...
|---|---|---:|---:|---:|---:|
| 1 | [scr/DateConversionCore/DateConversionCore.cpp](../scr/DateConversionCore/DateConversionCore.cpp) | 100.00% | 100 | 23 | 23.00 |
...
- This reassessment supersedes the `-01` report for current measured coverage.
- `DateConversionCore.cpp` now has full measured line and branch coverage under the macOS CMake test path.
- `Date_Conversion.cpp` remains 0% in this run because tests still target the extracted core logic, not MFC dialog handlers.
